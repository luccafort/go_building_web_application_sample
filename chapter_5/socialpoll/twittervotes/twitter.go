...
// stopchan は受信専用のシグナルチャネル
// votes は投票内容が送信されるシグナルチャネル
// 戻り値の stoppedchan はgoroutineの完了を伝える
func startTwitterStream(stopchan <-chan struct{}, votes chan<- string) <-chan struct{} {
	...
	go func() {
		...
		for {
			select {
			case <-stopchan: // チャネルへのメッセージがあれば処理を終了して戻る
				log.Println("Twitterへの問い合わせを終了します...")
				// return されるのでstoppedchanのdefer定義が実行される
				return
			default:
